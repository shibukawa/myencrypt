...
MyEncrypt は常に challenge 検証をスキップします。ACME クライアントが challenge を accept した直後に、MyEncrypt は challenge を即座に `valid` として扱います。

この挙動はローカル開発用途を前提としています。HTTP-01、TLS-ALPN-01、DNS-01 の実通信確認は行いません。
